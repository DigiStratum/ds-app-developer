...
export function DSHeader({
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 ${className}`}>
      ...
      {/* Mobile menu - visible on all screen sizes when open */}
      {showMobileMenu && (
        <div className="border-t border-gray-200 dark:border-gray-700">
          <div className="px-4 py-3 space-y-3">
            ...
            {/* Guest: Sign In - mobile */}
            {!isAuthenticated && showUserMenu && auth && (
              <button
                onClick={() => { auth.login(); setShowMobileMenu(false); }}
                className="flex items-center w-full px-4 py-3 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md"
              >
                {t('auth.signIn', 'Sign In')}
              </button>
            )}
                  ...
                  <div className="pb-3 border-b border-gray-200 dark:border-gray-700">
                    ...
                    >
                      ...
                      <div className="ml-3">
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{userName || user.email}</p>
                        {user.name && <p className="text-xs text-gray-500 dark:text-gray-400">{user.email}</p>}
                        <p className="text-xs text-blue-500 dark:text-blue-400">{t('nav.manageAccount', 'Manage account')} →</p>
                      </div>
                    </a>
                  </div>
                )}
