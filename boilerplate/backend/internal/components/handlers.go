...
// GetComponentHandler handles GET /api/components/{name}
// Returns component metadata and versions.
func (h *Handler) GetComponentHandler(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	// Extract component name from path
	name := extractPathParam(r.URL.Path, "/api/components/")
	
	// Handle case where version is also in path (e.g., /api/components/name/1.0.0)
